  ...
  return (
    <div className="relative mx-auto max-w-5xl px-5 py-12">
      <div
        ...
        aria-hidden
      >
        <div className="absolute -top-24 left-1/4 h-72 w-96 rounded-full bg-grape-200/40 blur-3xl" />
      </div>
      ...
      <div className="relative mt-6 overflow-hidden rounded-3xl bg-gradient-to-br from-grape-900 via-grape-800 to-grape-900 p-7 text-white shadow-candy md:p-8">
        <div className="candy-float pointer-events-none absolute -right-10 -top-10 h-44 w-44 rounded-full bg-bubble-500/20 blur-2xl" aria-hidden />
        <div className="relative flex flex-wrap items-start justify-between gap-6">
          <div className="flex flex-wrap items-center gap-4">
            <ChildAvatar child={child} size="lg" />
            <div>
              <p className="font-display text-xs font-bold uppercase tracking-widest text-grape-200">
                学习分析报告
              </p>
              <h1 className="mt-1 font-display text-3xl font-extrabold">{child.name}</h1>
              <p className="mt-2 text-sm text-grape-100/90">
          ...
          <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm backdrop-blur-sm">
            <p className="text-grape-200">最近练习</p>
            <p className="mt-1 font-bold text-white">
              {lastSession ? format(lastSession.createdAt, "yyyy-MM-dd HH:mm") : "尚未开始"}
            </p>
            <Link
              href={`/learn/${childId}`}
              className="mt-3 inline-flex text-sm font-bold text-sun-300 transition hover:text-sun-200"
            >
              继续练习 →
            </Link>
          </div>
      ...
      <section className="mt-8">
        <div className="mb-4">
          <h2 className="font-display text-lg font-extrabold text-slate-900">核心指标</h2>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-5">
        <Card className="lg:col-span-3">
          <h2 className="font-display text-lg font-extrabold text-slate-900">表现趋势</h2>
        ...
        <Card className="lg:col-span-2">
          <h2 className="font-display text-lg font-extrabold text-slate-900">键位热力图</h2>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <Card>
          <h2 className="font-display text-lg font-extrabold text-slate-900">键位诊断明细</h2>
          <p className="mt-1 text-sm text-slate-500">
          ...
          {report.adjacentPairs.length > 0 ? (
            <div className="mt-4 rounded-2xl border border-sun-200 bg-sun-50 p-4 text-sm text-sun-700">
              <p className="font-bold">相邻键误触分析</p>
              <p className="mt-1">
                误触占全部错误约 {Math.round(report.adjacentErrorRate * 100)}%，常见组合：
                {report.adjacentPairs.join("、")}
              </p>
              <p className="mt-2 text-xs text-sun-700/80">
        ...
        <Card>
          <h2 className="font-display text-lg font-extrabold text-slate-900">模式进度</h2>
      ...
      <Card className="mt-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="font-display text-lg font-extrabold text-slate-900">AI 学习报告{reportUnlocked ? "" : "（付费版）"}</h2>
            <p className="mt-1 text-sm text-slate-500">AI 分析练习数据，生成个性化学习建议与改进方向。</p>
          </div>
        </div>
        {reportUnlocked && child.aiRecommendations[0] ? (
          <div className="mt-4 rounded-2xl border-2 border-white bg-gradient-to-br from-grape-50 to-bubble-50 p-5 ring-1 ring-grape-100">
            <p className="text-xs font-bold uppercase tracking-wide text-grape-700">✨ 最新分析</p>
